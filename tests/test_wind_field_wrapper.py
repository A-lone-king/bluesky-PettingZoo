...
def _make_mock_env(
    ...
    # Build observations
    obs = {aid: {"self_state": np.zeros(6, dtype=np.float32)} for aid in agents}
...
class TestNoAugmentObs:
    ...
    def test_no_augment_obs_unchanged(self, mock_bs: MagicMock) -> None:
        ...
        obs, _ = wrapper.reset()
        for agent_id in env.agents:
            assert "wind_u" not in obs[agent_id], (
                "wind_u should not be added when augment_obs=False"
            )
            assert "wind_v" not in obs[agent_id], (
                "wind_v should not be added when augment_obs=False"
            )
