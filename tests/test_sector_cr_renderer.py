...
class TestSectorCRRendererFrame:
    """Verify SectorCR renders aircraft, sector polygon, and waypoints."""

    @patch("bluesky_pettingzoo.rendering.common.pygame")
    @patch("bluesky_pettingzoo.rendering.sector_cr_renderer.pygame")
    @patch("bluesky_pettingzoo.rendering.base_renderer.pygame")
    def test_render_frame_draws_aircraft_and_sector(self, mock_base_pygame, mock_scr_pygame, mock_common_pygame):
        ...
        # Should draw aircraft polygons + sector polygon
        assert mock_common_pygame.draw.polygon.call_count >= 3
        renderer.close()

    @patch("bluesky_pettingzoo.rendering.common.pygame")
    @patch("bluesky_pettingzoo.rendering.sector_cr_renderer.pygame")
    @patch("bluesky_pettingzoo.rendering.base_renderer.pygame")
    def test_render_frame_without_sector(self, mock_base_pygame, mock_scr_pygame, mock_common_pygame):
        mock_screen = MagicMock()
        mock_base_pygame.display.set_mode.return_value = mock_screen
        mock_base_pygame.Surface = MagicMock
        mock_base_pygame.font.Font.return_value = MagicMock()
        ...
        # Should draw aircraft but no sector polygon
        assert mock_common_pygame.draw.polygon.call_count >= 1
        renderer.close()
