...
将 BlueSky 空中交通仿真平台从单智能体环境（bluesky-gym）扩展为多智能体环境，构建符合 PettingZoo `ParallelEnv` 标准的 ATM 多智能体强化学习环境。BlueSky 是底层仿真引擎，本项目在其上构建多智能体 RL 接口。第一版聚焦于**飞机级冲突解脱与航线重构**，为后续扇区动态管理、LLM 集成奠定基础。
